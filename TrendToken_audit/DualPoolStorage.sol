...
contract DualPoolIntegration is DualPoolStorage {//is DualPoolTokens { // 6,800 bytes --> 5500 --> 4100
    ...
    /** 
     * @notice Allows for CompTT to be updated
     * @dev CompTT is the comptroller governing all Trend Tokens
     * @param _compDP The Dual Pool comptroller address
    */
    function _newCompDPinternal(address _compDP) internal {
        compDP = ICompDP(_compDP);
    }
   ...
   // ----- CONTRACT ACCOUNT DATA ------ // 
    ...
    /**
     * @notice Returns the amount of XDP accrued
     * @dev Amount gets updated when someone calls claimXDP() on CompDP
     */
    function amtAccruedXDP() internal view returns(uint) {
        uint value = compDP.venusAccrued(address(this));
        return value;
    }
