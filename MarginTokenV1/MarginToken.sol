...
contract MarginToken is VenusIntegration, MarginTokenStorage { 
    ...
    constructor(address _compTT) VenusIntegration(_compTT) public {
        manager = msg.sender;
    ...
    /**
     * @notice Sets new Trend Token comptroller, and incentive model
     ...
     */
    function _updateCompAndIncentive(address _compTT, IIncentiveModel _incentiveModel) onlyManager requireUnlocked external {
        
        address oldCompTT = address(compTT);
        if (_compTT != address(0)) {
            // compTT must support this Trend Token before being updated
            //require(ICompTT(_compTT).trendTokenActiveStatus(address(this)),"InactiveTT." );
        ...
        emit UpdateCompAndIncentive(oldCompTT, address(compTT), oldIncentive, address(incentiveModel));
    ...
    function _setDesiredAllocationsFresh(uint[] memory _con, uint[] memory _col, uint[] memory _bor) internal {
        ...
        require(conTotal.add(colTotal).sub(borTotal) == 1e18 && conTotal >= 0.01e18, "!=100%||con<1%");
     ...
     * param _newPortfolio The new portfolio 
    ...
    function _updatePortfolioAndAllocations(address[] calldata _newPortfolio, uint[] calldata _con, uint[] calldata _col, uint[] calldata _bor) onlyTradingBot external {
        ...
        for (uint i=0; i<portfolioTokens.length; i++) {
            ...
            if (!tokenInNewPortfolio) {
                ...
                requireUnderAmount(con.add(col).add(bor), maxDisableTokenValue, "!maxDisableValue.");
    ...
    function _claimXVSandFees(bool claimXVS, bool claimPerformanceFee, uint redeemAmtTrendToken) onlyManager external  {
        ...
        if (claimXVS) {
            ...
            uint balChangeXVS = balanceHolder(xvs,address(this)).sub(balBeforeXVS);
            if (balChangeXVS > 0) {
    ...
    */
    function storedEquity() internal view returns(address[] memory,uint[] memory,uint[] memory,uint[] memory,uint[] memory,uint) { 
        ...
        // assumes will always be positive, overflow otherwise
    ...
    */
    ...
    function trendTokenInCalculations(IERC20 _redeemBep20, uint _redeemAmt) 
        ...
        uint outValueAddFees = Lib.getValue(outValueEst, uint(int(1e18).add(feeOrReward))); 
    ...
    function redeemFresh(IERC20 _redeemBep20, uint _redeemAmt, uint _maxTrendTokenIn) internal pausedTrendToken returns(uint) { // change back to external
        ...
        trendToken.burn(trendTokenInAmt.sub(trendTokenFee));
    ...
    /**
     * @notice If Venus not open then only tradingBot can execute interactions with Venus
     */
