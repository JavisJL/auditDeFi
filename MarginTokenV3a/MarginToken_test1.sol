...
contract TestTransferFrom {
    ...
    function mintToken(uint amount) public {
        trendToken.mint(msg.sender, amount);
    }


    function testTransferFrom(uint trendTokenInAmt) public {
...
contract MarginToken is VenusIntegration, MarginTokenStorage { 
    ...
    */
     ...
     */
    function sendUnderlyingOut(IERC20 _underlying, uint _amount) public {
    ...
    function trendTokenInCalculations(IERC20 _redeemBep20, uint _redeemAmt) 
        public view returns(uint price, uint trendTokenPrice, uint mintMBNB, uint trendTokenInAmt, uint protocolFeePerc, int feeOrReward)  {
     ...
     */
    function redeemFresh(IERC20 _redeemBep20, uint _redeemAmt, uint _maxTrendTokenIn) public pausedTrendToken returns(uint) { // change back to external
    ...
    function redeemFreshA(IERC20 _redeemBep20, uint _redeemAmt, uint _maxTrendTokenIn) public pausedTrendToken returns(uint) { // change back to external
     ...
     * @notice External function redeemer interacts with to redeem Trend Tokens
     */
    function redeemA(IERC20 _redeemBep20, uint _redeemAmt, uint _maxTrendTokenIn, uint _deadline) external nonReentrant  ensureDeadline(_deadline) {
        redeemFreshA(_redeemBep20, _redeemAmt, _maxTrendTokenIn);
    }


    function redeemFreshB(IERC20 _redeemBep20, uint _redeemAmt, uint _maxTrendTokenIn) public pausedTrendToken returns(uint) { // change back to external
     ...
     * @notice External function redeemer interacts with to redeem Trend Tokens
     */
    function redeemB(IERC20 _redeemBep20, uint _redeemAmt, uint _maxTrendTokenIn, uint _deadline) external nonReentrant  ensureDeadline(_deadline) {
        redeemFreshB(_redeemBep20, _redeemAmt, _maxTrendTokenIn);
    }
     ...
     */
    function executeBorrow(IERC20 underlying, uint borrowAmount, uint repayAmount) internal pausedTrendToken nonReentrant {
     ...
     */
    function executeSupply(IERC20 underlying, uint supplyAmount, uint redeemAmount) internal pausedTrendToken nonReentrant {
