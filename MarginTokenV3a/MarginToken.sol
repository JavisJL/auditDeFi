...
// add permission to supply, redeem, borrow, and repay to public from admin
...
contract MarginToken is VenusIntegration, MarginTokenStorage { 
     ...
     * param _reset May be reset annually to reset the ATH to current prices
     */
    function _updatePerformanceFee(uint _performanceFee, bool _reset) onlyManager requireUnlocked external {
        ...
        // resets trendTokenATH to current prices
        if (_reset) {
           trendTokenATH = trendTokenPrice;
        }
    ...
    */
    ...
    function executeBorrow(IERC20 underlying, uint borrowAmount, uint repayAmount) external pausedTrendToken nonReentrant {
        ...
        if (borrowAmount>0) {
            ...
            address vToken = compTT.permissionBorrow(address(this), address(underlying), msg.sender, borrowAmount);
            ...
            address vToken =  compTT.permissionRepay(address(this), address(underlying), msg.sender, repayAmount);
    ...
    function executeSupply(IERC20 underlying, uint supplyAmount, uint redeemAmount) external pausedTrendToken nonReentrant {
        ...
        if (supplyAmount > 0) {
            ...
            address vToken = compTT.permissionSupply(address(this), address(underlying), msg.sender, supplyAmount);
            ...
            // checks borrow factor, borrow direction, and active status
            address vToken = compTT.permissionRedeem(address(this), address(underlying), msg.sender, redeemAmount);
