...
contract TrendLensStorage {
    ...
    /**
     * mainnet: 0x64992F2E63Ffe08d346269d740d131830Df8F10b
     * testnet: 0xb2Ab79C452bD242862C7593A076833350CD73fB8
     */
    ICompTT public compTT = ICompTT(0x64992F2E63Ffe08d346269d740d131830Df8F10b);
...
contract TrendLens is TrendLensStorage {
    ...
    /** 
     * @notice Calculates swappairs in place of zero address
     * @dev Used for frontend to generate swappairs in place of zero address
     * @param trendToken The Trend Token (or Margin Token) contract address
     * @param inputToken The input underlying token or trend token contract address (if redeem) 
     * @param outputToken The output underlying token or trend token contract address (if deposit)  
     * @return portfolioPlusTrendTokenRemoveInputAndOutput The list of available tokens for swap
     * example 0x7d55CB2Ae870D8EDa9bD896eA057a74D04905DF8,0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd,0xA808e341e8e723DC6BA0Bb5204Bafc2330d7B8e4
     */
    ...
    /**
     * @notice Emits an error message when insufficient output token or max supply exceeded
     */
    function amountsOutErrorMessage(IMarginToken trendTokenContract, IERC20 tokenOut, uint amountOutCalulated) public view {

        // get contract balance of tokenOut
        IERC20 trendTokenTkn = IERC20(address(trendTokenContract.trendToken()));
        if (tokenOut == trendTokenTkn) {

            uint maxSupply = trendTokenContract.maxSupply();
            uint currentSupply = trendTokenTkn.totalSupply();
            require(currentSupply + amountOutCalulated < maxSupply, "Deposit would exceed trend token max supply. Please reduce deposit.");


        } else {

            uint balanceContract = walletBalance(address(trendTokenContract), tokenOut);
            require(amountOutCalulated<= balanceContract,"Insufficient balance of output token in Trend Token contract. Please reduce quantity or buy another asset.");
    ...
    function amountsOut(IMarginToken trendTokenContract, IERC20 tokenIn, IERC20 tokenOut, uint amount, bool directionIn, address trader) 
        ...
        amountsOutErrorMessage(trendTokenContract, tokenOut, amountOut);
