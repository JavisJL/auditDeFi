...
contract CompTT is ComptrollerStorage {
    ...
    function getXTTAddress() external pure returns(address) {
        return 0xdAF80F27b5d8d2bbd9a88702946D296c7c343f74;
    }
    ...
    function _supportTrendToken(IMarginToken trendToken) external requireUnlocked returns (bool) {
        ...
        trendTokens[address(trendToken)] = TrendToken({ isLocked: false, // allows manager to make key changes
                                                        ...
                                                        maxBorrowFactor: 0.5e18, // trend token may borrow maximum 50% of borrowable   
    ...
    function _supportUnderlying(address underlying) external requireUnlocked returns(bool) {
        ...
        underlyingInfo[underlying] = Underlying({   isActive: true, // underlying is active (may be added to Trend Tokens)
                                                    ...
                                                    isVenusOpen: false, // public cannot rebalance Venus balances
     ...
     */
    function _updateUnderlyingForVenusActions(address underlying, bool _isVenusActive, bool _isVenusOpen, bool _isSupplyVenus, bool _isBorrowVenus) external requireUnlocked onlySupportedUnderlyins(underlying) {
        ...
        underlyingInfo[underlying].isVenusOpen = _isVenusOpen;
     ...
     */
    function underlyingForVenusActions(address underlying) external view onlyProtocolAllowed returns(bool,bool,bool,bool) {
        return (underlyingInfo[underlying].isVenusActive,
                underlyingInfo[underlying].isVenusOpen,
    ...
    function checkTradeValue(address trendToken, address underlying, uint amount) internal view  {
        ...
        uint valueOut = amount * price / 1e18;
    ...
    function permissionTrade(address trendToken, address underlyingIn, address underlyingOut, uint valueIn, uint valueOut) external view onlyProtocolAllowed onlySupportedTrendTokens(trendToken) returns(bool) {
        ...
        uint maxTradeFee = trendTokens[trendToken].maxTradeFee;
        require(valueOut < valueIn * (uint(1e18) + maxTradeFee) / 1e18 && 
                valueOut > valueIn * (uint(1e18) - maxTradeFee) / 1e18,
                "!maxTradeFee");
        ...
        // ensure permission to trade assets with Trend Token 
        require(trendTokens[trendToken].isTrade && underlyingInfo[underlyingIn].isTrade && underlyingInfo[underlyingOut].isTrade,"!isTrade.");

        // ensures permission to deposit one asset and redeem the other
        require(underlyingInfo[underlyingIn].isDeposit && underlyingInfo[underlyingOut].isRedeem,"!isDeposit and isRedeem");
     ...
     * @return approved True if supply or redeem brings current supplies closer to desired, otherwise false
     */
    function performSupplyDirectionCheck(address trendToken, address underlying, uint supplyAmount, uint redeemAmount) internal view returns(bool) {
        ...
        uint currentSupplyAmount = col[underlyingIndex] * 1e18 / price;
        require(redeemAmount <= currentSupplyAmount, "redeem exceeded supply amount.");
        uint afterSupplyAmount = currentSupplyAmount + supplyAmount - redeemAmount; // invalid repayAmount if negative (error)

        // get desired supply amount
        uint desiredSupplyPercent = IMarginToken(trendToken).collateralAllo(underlyingIndex);
        uint desiredSupplyAmount = desiredSupplyPercent * equity / price;
    ...
    /**
     * @notice Restricts underlyings' Venus rebalances to tradingBot if isVenusOpen is false for specific underlying
     * @dev Used to determine if caller can rebalance Trend Tokens' Venus positions
     */
    function rebalanceVenusAllowed(IMarginToken trendToken, address underlying, address caller) internal view {

        // require venus is open (call to public and trading bot) or caller is tradingBot
        require(underlyingInfo[underlying].isVenusOpen || caller != trendToken.tradingBot(),"!venusOpen");
     ...
     */
    function permissionSupply(address trendToken, address underlying, address caller, uint supplyAmount) external view onlyProtocolAllowed onlySupportedTrendTokens(trendToken) returns(address) {
        ...
        // tradingBot must call if isVenusOpen is false
        require(underlyingInfo[underlying].isVenusOpen || caller == IMarginToken(trendToken).tradingBot(),"!venusOpen");
        ...
        // ensure permission to deposit asset to Trend Token 
        require(trendTokens[trendToken].isDeposit && underlyingInfo[underlying].isDeposit,"!isDeposit");
     ...
     * @return vToken if permission is granted, otherwise zero address
     */
    function permissionRedeem(address trendToken, address underlying, address caller, uint redeemAmount) external view onlyProtocolAllowed onlySupportedTrendTokens(trendToken) returns(address) {
        ...
        // ensure permission to redeem asset from Trend Token 
        require(trendTokens[trendToken].isRedeem && underlyingInfo[underlying].isRedeem,"!isRedeem");
    ...
    function getStoredBorrowsTotal(address trendToken) internal view returns(uint) {
        ...
        for (uint i = 0; i < vTokens.length; i ++) {
            ...
            storedBorrows = storedBorrows + vToken.borrowBalanceStored(trendToken);
        }
    ...
    function getHypoAccountLiquidity(address trendToken, address vToken, uint redeemAmount, uint borrowAmount) internal view returns(uint, uint) {
        ...
        uint redeemTokens = redeemAmount * 1e18 / exchangeRate; // the amount of vTokens to be redeemed
    ...
    function performBorrowFactorCheck(address trendToken, address underlying, address vToken, uint redeemAmount, uint borrowAmount) internal view returns(bool) {
        ...
        uint borrowValue = borrowAmount * priceUnderlyingFresh(IERC20(underlying)) / 1e18;
        uint afterStoredBorrowsTotal = currentStoredBorrowsTotal + borrowValue;

        // calculate borrow factor after action 'borrows/borrowable'
        uint afterBorrowFactor = 0;
        if (afterStoredBorrowsTotal>0) {
            uint afterBorrowable = afterStoredBorrowsTotal + afterLiquidity;
            afterBorrowFactor = afterStoredBorrowsTotal * 1e18 / afterBorrowable;
        }
    ...
    function performBorrowDirectionCheck(address trendToken, address underlying, uint borrowAmount, uint repayAmount) internal view returns(bool) {
        ...
        uint desiredBorrowAmount = desiredBorrowPercent * equity / price;

        // get before borrow amount
        uint currentBorrowAmount = bor[underlyingIndex] * 1e18 / price;
        require(repayAmount <= currentBorrowAmount + borrowAmount, "repay > borrow amount");
        uint afterBorrowAmount = currentBorrowAmount + borrowAmount - repayAmount; // invalid repayAmount if negative (error)
     ...
     * @return vToken if permission is granted, otherwise zero address
     */
    function permissionBorrow(address trendToken, address underlying, address caller, uint borrowAmount) external view onlyProtocolAllowed onlySupportedTrendTokens(trendToken) returns(address) {
     ...
     * @return vToken if permission is granted, otherwise zero address
     */
    function permissionRepay(address trendToken, address underlying, address caller, uint repayAmount) external view onlyProtocolAllowed onlySupportedTrendTokens(trendToken) returns(address) {
